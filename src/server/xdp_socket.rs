...
fn xsk_alloc_umem_frame(umem_frame_addr: &mut [u64], umem_frame_free: &mut usize) -> u64 {
...
pub(crate) unsafe fn peek_rx_ring(
    ...
    umem_frame_free: &mut usize,
    umem_frame_addr: &mut [u64],
) -> u64 {
    ...
    if stock_frames > 0 {
        ...
        println!(
            "xsk_ring_prod__reserve: {}, stock_frames: {}, idx_fq: {}, umem_frame_free: {}",
            ret, stock_frames, idx_fq, umem_frame_free
        );

        for _ in 0..stock_frames {
            _xsk_ring_prod__fill_addr(fq, *idx_fq)
